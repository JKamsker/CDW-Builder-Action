    ...
    public static class HostBuilderExtensions
    {
            ...
            where TStartup : class
        {
            ...
            hostBuilder.ConfigureServices((ctx, serviceCollection) =>
            {
                ...
                var cfgServicesMethod = typeof(TStartup)
                    .GetMethod(ConfigureServicesMethodName, new Type[] { typeof(IServiceCollection) });

                // Check if TStartup has a ctor that takes a IConfiguration parameter
                var hasConfigCtor = typeof(TStartup)
                    .GetConstructor(new Type[] { typeof(IConfiguration) }) != null;

                IConfiguration? config = ctx.Configuration;

                // create a TStartup instance based on ctor
                var startUpObj = hasConfigCtor ?
                    (TStartup)Activator.CreateInstance(typeof(TStartup), config) :
